...
import com.alant7_.util.event.PlayerEquipArmorEvent;
...
import com.alant7_.util.event.PlayerEquipArmorEvent.ArmorType;
import com.alant7_.util.event.PlayerEquipArmorEvent.EquipMethod;

public class EventListenerArmorEquip implements Listener {

    private final List<String> blockedMaterials;

    public EventListenerArmorEquip(List<String> blockedMaterials) {
        ...
        if (shift) {
            ...
            if (newArmorType != null) {
                ...
                if (newArmorType.equals(ArmorType.HELMET) && (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getHelmet()) : !isAirOrNull(e.getWhoClicked().getInventory().getHelmet())) || newArmorType.equals(ArmorType.CHESTPLATE) && (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getChestplate()) : !isAirOrNull(e.getWhoClicked().getInventory().getChestplate())) || newArmorType.equals(ArmorType.LEGGINGS) && (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getLeggings()) : !isAirOrNull(e.getWhoClicked().getInventory().getLeggings())) || newArmorType.equals(ArmorType.BOOTS) && (equipping ? isAirOrNull(e.getWhoClicked().getInventory().getBoots()) : !isAirOrNull(e.getWhoClicked().getInventory().getBoots()))) {
                    PlayerEquipArmorEvent armorEquipEvent = new PlayerEquipArmorEvent((Player) e.getWhoClicked(), EquipMethod.SHIFT_CLICK, newArmorType, equipping ? null : e.getCurrentItem(), equipping ? e.getCurrentItem() : null);
            ...
            if (numberkey) {
                if (e.getClickedInventory().getType().equals(InventoryType.PLAYER)) {// Prevents shit in the 2by2 crafting
                    ...
                    if (!isAirOrNull(hotbarItem)) {// Equipping
                        newArmorType = PlayerEquipArmorEvent.ArmorType.matchType(hotbarItem);
                        newArmorPiece = hotbarItem;
                        oldArmorPiece = e.getClickedInventory().getItem(e.getSlot());
                    } else {// Unequipping
                        newArmorType = PlayerEquipArmorEvent.ArmorType.matchType(!isAirOrNull(e.getCurrentItem()) ? e.getCurrentItem() : e.getCursor());
                    }
                }
            } else {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (newArmorType != null) {
                if (newArmorType.equals(ArmorType.HELMET) && isAirOrNull(e.getPlayer().getInventory().getHelmet()) || newArmorType.equals(ArmorType.CHESTPLATE) && isAirOrNull(e.getPlayer().getInventory().getChestplate()) || newArmorType.equals(ArmorType.LEGGINGS) && isAirOrNull(e.getPlayer().getInventory().getLeggings()) || newArmorType.equals(ArmorType.BOOTS) && isAirOrNull(e.getPlayer().getInventory().getBoots())) {
                    PlayerEquipArmorEvent armorEquipEvent = new PlayerEquipArmorEvent(e.getPlayer(), EquipMethod.HOTBAR, ArmorType.matchType(e.getItem()), null, e.getItem());
                    Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
                    if (armorEquipEvent.isCancelled()) {
                        e.setCancelled(true);
    ...
    public void inventoryDrag(InventoryDragEvent event) {
        ...
        if (type != null && type.getSlot() == event.getRawSlots().stream().findFirst().orElse(0)) {
            PlayerEquipArmorEvent armorEquipEvent = new PlayerEquipArmorEvent((Player) event.getWhoClicked(), EquipMethod.DRAG, type, null, event.getOldCursor());
            Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
            if (armorEquipEvent.isCancelled()) {
    ...
    public void itemBreakEvent(PlayerItemBreakEvent e) {
        ...
        if (type != null) {
            ...
            PlayerEquipArmorEvent armorEquipEvent = new PlayerEquipArmorEvent(p, EquipMethod.BROKE, type, e.getBrokenItem(), null);
            Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
            if (armorEquipEvent.isCancelled()) {
    ...
    public void playerDeathEvent(PlayerDeathEvent e) {
        ...
        for (ItemStack i : p.getInventory().getArmorContents()) {
            if (!isAirOrNull(i)) {
                Bukkit.getServer().getPluginManager().callEvent(new PlayerEquipArmorEvent(p, EquipMethod.DEATH, ArmorType.matchType(i), i, null));
    ...
    public void dispenseArmorEvent(BlockDispenseArmorEvent event){
        ...
        if(type != null){
            if(event.getTargetEntity() instanceof Player){
                ...
                PlayerEquipArmorEvent armorEquipEvent = new PlayerEquipArmorEvent(p, EquipMethod.DISPENSER, type, null, event.getItem());
                Bukkit.getServer().getPluginManager().callEvent(armorEquipEvent);
                if(armorEquipEvent.isCancelled()){
                    event.setCancelled(true);
                }
